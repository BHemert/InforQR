...
documentName = 'qr'  # Set document name where qr codes will be saved
...
list = [  # "[LOCATION] [AMOUNT OF QR CODES]"
    ...
    # EWENT / EMINENT
    "AEI 8", "AEJ 5", "AEK 5", "AEL 5", "AEM 5", "AEN 5", "AEO 5", "AEP 5", "AEQ 5",
...
def docxQR():
    for loc in list:
        ...
        for num in range(int(totNum)):
            ...
            run.add_picture("./QRCODES/" + str(loc[0]) + "/" + str(loc[0]) + str(
                num) + ".png", width=Inches(0.6), height=Inches(0.6))
    ...
    print("Succesfully Saved all qr codes in: " +
          os.path.abspath('./' + documentName + '.docx'))
...
def makeQR():
    for loc in list:
        ...
        for num in range(int(totNum) + 1):
            ...
            qr.make(fit=True)
            img = qr.make_image()
            if (os.path.exists("./QRCODES/" + str(loc[0]))):
                img.save("./QRCODES/" +
                         str(loc[0]) + "/" + str(loc[0]) + str(num) + ".png")
